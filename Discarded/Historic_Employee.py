## The original employee page using historic data 
...
#from THAW import employee

voucher = incFB[['Transaction Code', 'Date', 'Value', 'Recipient','Type','Sender','Admin Cost', 'Delivery Method', 'Time', 'Postcode']]
# voucher['Date'] = pd.to_datetime(voucher['Date'], format = '%d/%m/%Y', errors='coerce')

fundEmp = funding2[['Date', 'Value', 'Funded By']]

# funds['Date'] = pd.to_datetime(funds['Date'], format = '%d/%m/%Y', errors='coerce')


# following code outputs info for clients who have received more than one voucher from THAW
...
# calculate current total of all income received since 01/01/2020
...
def acc(storeAcc):
    ...
    StoreName = st.selectbox('Which store account do you want to see:', ['Papdale', 'Charis'])
    ...
    due['Value'] = due['Value'].astype('float') # astype('int32') gives error
